...
export default function FuelTankPWAPrototype() {
  ...
  const selectStation = (stationId, clearReading = true) => {
    ...
    if (clearReading) { setReadingMm(""); setUnloadInitialMm(""); setUnloadFinalMm(""); }
  };

  const saveReading = () => {
    if (!hasReading || readingHasError) return;
    persistHistory([{ id: makeId(), stationId: selectedStationId, date: new Date().toLocaleString(), station: station.name, tank: tank.name, product: tank.product, mm: readingNumber, liters, percentage, ullage, operator: operator || "Not entered" }, ...history]);
  };

  const resetUnloadingForm = () => { setUnloadInitialMm(""); setUnloadFinalMm(""); setTruckInvoiceLiters(""); setUnloadReference(""); setTruckPlate(""); setDriverName(""); setEditingUnloadingId(null); };
  ...
  const saveUnloading = () => {
    if (!unloading.hasValidReadings || unloadReadingHasError) return;
    const existing = unloadingHistory.find((row) => row.id === editingUnloadingId);
    const record = { id: existing?.id || makeId(), stationId: selectedStationId, deliveryId: currentDeliveryId, date: existing?.date || new Date().toLocaleString(), updatedAt: editingUnloadingId ? new Date().toLocaleString() : undefined, station: station.name, tank: tank.name, product: tank.product, initialMm: unloadInitialNumber, finalMm: unloadFinalNumber, initialLiters: unloading.initialLiters, finalLiters: unloading.finalLiters, deliveredLiters: unloading.deliveredLiters, invoiceLiters: unloading.invoiceLiters, variance: unloading.variance, reference: unloadReference || "Not entered", truckPlate: truckPlate || "Not entered", driverName: driverName || "Not entered", operator: operator || "Not entered" };
    if (editingUnloadingId) { persistUnloadingHistory(unloadingHistory.map((row) => (row.id === record.id ? record : row))); resetUnloadingForm(); }
    else persistUnloadingHistory([record, ...unloadingHistory]);
  };
  ...
  return <div className="app-shell"><style>{styles}</style><div className="app-container">
    ...
    {activePage === "daily" ? <><main className="main-grid"><Card><div className="card-content form-grid"><div className="section-title"><span style={{ fontSize: 24 }}>⛽</span><h2>Enter Reading</h2></div>{tankSelector(true, false)}<div><FieldLabel>Reading in millimeters</FieldLabel><input className="field-input" type="number" inputMode="decimal" min="0" max={maxMm} placeholder={`Example: ${Math.round(maxMm / 2)}`} value={readingMm} onChange={(event) => setReadingMm(event.target.value)} /><p className="small-text">Maximum calibrated height: {maxMm} mm</p>{readingHasError ? <p className="small-text error-text">Check this reading. It must be within the calibrated height.</p> : null}</div><div><FieldLabel>Operator / staff name</FieldLabel><input className="field-input" placeholder="Optional" value={operator} onChange={(event) => setOperator(event.target.value)} /></div><button type="button" onClick={saveReading} disabled={!hasReading || readingHasError} className="primary-button">💾 Save Reading</button></div></Card><Card className={`results-card ${getProductClass(tank.product)}`}><div className="results-grid"><section className="results-info"><div><p className="selected-label">Selected station</p><h2 className="selected-title">{station.name}</h2><p className="selected-subtitle">{tank.name} • <ProductBadge product={tank.product} /> • Capacity {formatNumber(capacity)} L</p></div><div className="metric-grid"><div className="metric-box"><p className="metric-label">Liters</p><p className="metric-value">{formatNumber(liters)} L</p></div><div className="metric-box"><p className="metric-label">Available space</p><p className="metric-value">{formatNumber(ullage)} L</p></div><div className="metric-box"><p className="metric-label">Full</p><p className="metric-value">{formatNumber(percentage)}%</p></div><div className="metric-box"><p className="metric-label">Reading</p><p className="metric-value">{hasReading ? readingNumber : 0} mm</p></div></div></section><section className="tank-visual-section"><div className="tank-visual" style={{ "--tank-width": `${percentage}%` }}><div className={`tank-fill ${tankLevelInfo.className}`} style={{ height: `${percentage}%` }} /><div className="tank-center"><div className="tank-badge"><p style={{ margin: 0, fontSize: 20, fontWeight: 900 }}>{formatNumber(percentage)}%</p><p className="tank-caption">{tankLevelInfo.label}</p></div></div></div><span className="tank-status-badge" style={{ "--level-color": tankLevelInfo.color }}>{tankLevelInfo.label}</span></section></div></Card></main><Card><div className="card-content"><div className="history-header"><div><h2>Reading History</h2><p>Saved tank readings on this device/browser.</p></div><button type="button" onClick={() => persistHistory([])} className="secondary-button">🗑️ Clear</button></div><div className="history-table-wrap"><table className="history-table"><thead><tr><th>Date</th><th>Station</th><th>Tank</th><th>Product</th><th>MM</th><th>Liters</th><th>Level</th><th>Ullage</th><th>Operator</th></tr></thead><tbody>{stationReadingHistory.length === 0 ? <tr><td style={{ padding: "22px 8px", color: "#64748b" }} colSpan={9}>No readings saved yet for this station.</td></tr> : stationReadingHistory.map((row) => <tr key={row.id} className={`history-row-${getProductClass(row.product)}`}><td>{row.date}</td><td>{row.station}</td><td>{row.tank}</td><td><ProductBadge product={row.product} /></td><td>{row.mm}</td><td>{formatNumber(row.liters)} L</td><td><HistoryLevelVisual percentage={row.percentage} /></td><td>{formatNumber(row.ullage)} L</td><td>{row.operator}</td></tr>)}</tbody></table></div></div></Card></> : null}

    {activePage === "delivery" ? <><Card><div ref={unloadingSectionRef} className="card-content form-grid" style={{ scrollMarginTop: 24 }}><div className="section-title"><span style={{ fontSize: 24 }}>🚚</span><h2>Truck Unloading / Descarregamento</h2></div><p className="small-text" style={{ margin: 0 }}>Use this when receiving fuel. First choose the correct station, tank, and product, then enter the reading before and after unloading.</p><div className="unloading-grid">{tankSelector(false, true)}<div className={`metric-box ${getProductClass(tank.product)}`}><p className="metric-label">Selected product</p><p className="metric-value" style={{ fontSize: 20 }}>{tank.name} • {tank.product}</p></div></div>{editingUnloadingId ? <div className="edit-banner">Editing saved unloading record. Change the details below, then press Update Unloading.</div> : null}<div className="unloading-grid"><div><FieldLabel>Initial reading before unloading (mm)</FieldLabel><input className="field-input" type="number" inputMode="decimal" min="0" max={maxMm} value={unloadInitialMm} onChange={(event) => setUnloadInitialMm(event.target.value)} placeholder="Before truck unloads" /></div><div><FieldLabel>Final reading after unloading (mm)</FieldLabel><input className="field-input" type="number" inputMode="decimal" min="0" max={maxMm} value={unloadFinalMm} onChange={(event) => setUnloadFinalMm(event.target.value)} placeholder="After truck unloads" /></div><div><FieldLabel>Truck / invoice liters optional</FieldLabel><input className="field-input" type="number" inputMode="decimal" min="0" value={truckInvoiceLiters} onChange={(event) => setTruckInvoiceLiters(event.target.value)} placeholder="Example: 10000" /></div></div><div className="unloading-grid"><div><FieldLabel>Invoice / delivery note number</FieldLabel><input className="field-input" value={unloadReference} onChange={(event) => setUnloadReference(event.target.value)} placeholder="Example: INV123" /></div><div><FieldLabel>Truck plate</FieldLabel><input className="field-input" value={truckPlate} onChange={(event) => setTruckPlate(event.target.value)} placeholder="Example: ABC-123" /></div><div><FieldLabel>Driver name</FieldLabel><input className="field-input" value={driverName} onChange={(event) => setDriverName(event.target.value)} placeholder="Optional" /></div></div><div className="metric-grid"><div className="metric-box"><p className="metric-label">Initial liters</p><p className="metric-value">{formatNumber(unloading.initialLiters)} L</p></div><div className="metric-box"><p className="metric-label">Final liters</p><p className="metric-value">{formatNumber(unloading.finalLiters)} L</p></div><div className="metric-box"><p className="metric-label">Received from truck</p><p className="metric-value">{formatNumber(unloading.deliveredLiters)} L</p></div><div className="metric-box"><p className="metric-label">Difference</p><p className={`metric-value ${unloading.variance >= 0 ? "variance-positive" : "variance-negative"}`}>{formatNumber(unloading.variance)} L</p></div></div><p className="small-text">Truck Delivery ID: <strong>{currentDeliveryId}</strong></p>{unloadReadingHasError ? <p className="small-text error-text">Check unloading readings. Final reading must be higher than initial and within calibrated height.</p> : null}<div className="unloading-actions"><button type="button" onClick={saveUnloading} disabled={!unloading.hasValidReadings || unloadReadingHasError} className="primary-button">{editingUnloadingId ? "✅ Update Unloading" : "💾 Save Unloading"}</button><button type="button" onClick={resetUnloadingForm} className="secondary-button">Reset</button></div></div></Card><Card><div className="card-content"><div className="history-header"><div><h2>Unloading History</h2><p>Saved truck unloading records on this device/browser.</p></div><button type="button" onClick={() => persistUnloadingHistory([])} className="secondary-button">🗑️ Clear</button></div><div className="history-table-wrap"><table className="history-table"><thead><tr><th>Action</th><th>Date</th><th>Delivery ID</th><th>Station</th><th>Tank</th><th>Product</th><th>Initial MM</th><th>Final MM</th><th>Initial L</th><th>Final L</th><th>Received L</th><th>Invoice L</th><th>Diff L</th><th>Reference</th><th>Truck Plate</th><th>Driver</th><th>Operator</th></tr></thead><tbody>{stationUnloadingHistory.length === 0 ? <tr><td style={{ padding: "22px 8px", color: "#64748b" }} colSpan={17}>No unloading records saved yet for this station.</td></tr> : stationUnloadingHistory.map((row) => <tr key={row.id} className={`history-row-${getProductClass(row.product)}`}><td><button type="button" onClick={() => startEditUnloading(row)} className="secondary-button" style={{ minHeight: 34, padding: "7px 10px" }}>Edit</button></td><td>{row.date}</td><td>{row.deliveryId || createTruckDeliveryId(row.reference, row.truckPlate, row.driverName)}</td><td>{row.station}</td><td>{row.tank}</td><td><ProductBadge product={row.product} /></td><td>{row.initialMm}</td><td>{row.finalMm}</td><td>{formatNumber(row.initialLiters)} L</td><td>{formatNumber(row.finalLiters)} L</td><td>{formatNumber(row.deliveredLiters)} L</td><td>{formatNumber(row.invoiceLiters)} L</td><td className={row.variance >= 0 ? "variance-positive" : "variance-negative"}>{formatNumber(row.variance)} L</td><td>{row.reference}</td><td>{row.truckPlate || "Not entered"}</td><td>{row.driverName || "Not entered"}</td><td>{row.operator}</td></tr>)}</tbody></table></div></div></Card></> : null}
