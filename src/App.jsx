...
function buildReadingDifferenceReports(readingRows = [], fromDate = "", toDate = "") {
  const groups = new Map();
  ...
  const reports = [];
  groups.forEach((rows) => {
    const from = getLatestReadingOnOrBefore(rows, fromDate, "start");
    const to = getLatestReadingOnOrBefore(rows, toDate, "end");
...
export default function FuelTankPWAPrototype() {
  ...
  const readingMovementReports = useMemo(() => buildReadingDifferenceReports(stationReadingHistory, reportFromDate, reportToDate), [stationReadingHistory, reportFromDate, reportToDate]);
