...
function buildReadingMovementReports(readingRows = []) {
  const groups = new Map();
  ...
  groups.forEach((rows) => {
    ...
    for (let index = 1; index < sorted.length; index += 1) {
      ...
      const movement = endLiters - startLiters;
      reports.push({
        id: `${from.id}-${to.id}`,
        station: to.station,
        tank: to.tank,
        ...
        endLiters,
        movement,
        fuelOut: movement < 0 ? Math.abs(movement) : 0,
        fuelIn: movement > 0 ? movement : 0,
  ...
  return reports.sort((a, b) => getRowTime(b.toDate) - getRowTime(a.toDate));
}
...
export default function FuelTankPWAPrototype() {
  ...
  const readingMovementReports = useMemo(() => buildReadingMovementReports(filterRowsByDateRange(stationReadingHistory, reportFromDate, reportToDate)), [stationReadingHistory, reportFromDate, reportToDate]);
