...
function buildSelectedReadingDifferenceReport(readingRows = [], fromReadingId = "", toReadingId = "") {
  ...
  if (!from || !to || from.id === to.id || getRowTime(to) <= getRowTime(from)) return [];
...
export default function FuelTankPWAPrototype() {
  ...
  const [readingReportTank, setReadingReportTank] = useState("");
  const [readingReportFromId, setReadingReportFromId] = useState("");
  const [readingReportToId, setReadingReportToId] = useState("");
  ...
  const selectedReadingTankName = readingReportTank || readingReportTankOptions[0] || "";
  const readingReportOptions = useMemo(() => sortReadingsNewestFirst(stationReadingHistory.filter((row) => row.tank === selectedReadingTankName)), [stationReadingHistory, selectedReadingTankName]);
  const readingMovementReports = useMemo(() => buildSelectedReadingDifferenceReport(readingReportOptions, readingReportFromId, readingReportToId), [readingReportOptions, readingReportFromId, readingReportToId]);
  const latestReadingMovementReports = useMemo(() => buildLastTwoReadingReports(stationReadingHistory), [stationReadingHistory]);
  const readingMovementTotals = useMemo(() => readingMovementReports.reduce((totals, row) => {
    ...
    return totals;
  }, { fuelOut: 0, fuelIn: 0, dieselOut: 0, petrolOut: 0 }), [readingMovementReports]);
  ...
  useEffect(() => {
    if (!readingReportTank && readingReportTankOptions.length > 0) setReadingReportTank(readingReportTankOptions[0]);
  }, [readingReportTank, readingReportTankOptions]);

  useEffect(() => {
    if (readingReportOptions.length < 2) {
      setReadingReportFromId("");
      setReadingReportToId("");
      return;
    }
    const newest = readingReportOptions[0];
    const previous = readingReportOptions[1];
    if (!readingReportFromId || !readingReportOptions.some((row) => row.id === readingReportFromId)) setReadingReportFromId(previous.id);
    if (!readingReportToId || !readingReportOptions.some((row) => row.id === readingReportToId)) setReadingReportToId(newest.id);
  }, [readingReportOptions, readingReportFromId, readingReportToId]);
