...
const styles = `
  * { box-sizing: border-box; }
  ...
  @media (max-width:760px) {
    ...
    .status-row { display: flex; width: auto; gap: 10px; align-items:center; }
...
export default function FuelTankPWAPrototype() {
  ...
  return <div className="app-shell"><style>{styles}</style><div className="app-container">
    <header className="app-header"><div><h1 className="app-title" style={{fontWeight:900,letterSpacing:"1px"}}>FTL</h1><p className="app-subtitle">Fast mobile readings for liters, ullage, truck unloading, and delivery reports.</p></div><div className="status-row">{(permissions.changeStation && userStationId === "all") ? <select value={selectedStationId} onChange={(event) => selectStation(event.target.value, true)} className="field-input" style={{width:"125px",minHeight:"34px",fontSize:"8px",padding:"4px 18px 4px 8px",borderRadius:"999px",marginRight:"6px"}}>{availableStations.map(([id,item]) => <option key={id} value={id}>{item.name}</option>)}</select> : <div className="status-pill" style={{fontSize:"11px",padding:"8px 10px"}}>{station.name}</div>}<div className="user-menu-wrap"><button type="button" className="user-circle" onClick={() => setUserMenuOpen((open) => !open)}>{String(loggedInUser.username || "U").slice(0, 1).toUpperCase()}</button>{userMenuOpen ? <div className="user-dropdown"><div><p className="user-dropdown-title">{loggedInUser.username}</p><p className="user-dropdown-subtitle">{loggedInUser.role}</p></div><button type="button" onClick={handleLogout} className="logout-button">Logout</button></div> : null}</div></div></header>
